...
gene = 'ERBB2';
idx_rna = find(~cellfun(@isempty,(strfind(TCGA_RNAseq_v1.hgnc(RNAseqIDs,1),gene))));
...
addpath('H2out_TCGA') % Add the new directory to the PATH
notinthere = (1:4000)';
parfor rr = 1:rtimes % Run on parallel
    ...
    if ~any(rr==notinthere)
        ...
        parsavef(['H2out_TCGA/TLas1_r' num2str(rr)],TLas1,BLas1,a0Vals1,YYpg,YYpgdev); % See the directory naming above
