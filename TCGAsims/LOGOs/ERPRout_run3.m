...
%%%%% Find HER2-normal, ER/PR-pos samples
gene = 'ERBB2';
idx_rna = find(~cellfun(@isempty,(strfind(RNAseqGenes,gene))));
H2lowIDs = find(XX(idx_rna,:)<7); % 7 is the break point in the histogram
gene = 'PGR';
idx_rna = find(~cellfun(@isempty,(strfind(RNAseqGenes,gene))));
PRhighIDs = find(XX(idx_rna,:)>3); 
gene = 'ESR1';
idx_rna = find(~cellfun(@isempty,(strfind(RNAseqGenes,gene))));
ERhighIDs = find(XX(idx_rna,:)>3); 
ERPRpids1 = intersect(H2lowIDs,PRhighIDs);
ERPRpids2 = intersect(ERPRpids1,ERhighIDs);
...
rtimes = 6000; % Number of Lasso modules to run
CVnfolds = 4; % Cross-validation number for glmnet package
% mkdir H2out_TCGA % Create a new directory for outputs. Naming according to input data is advised
addpath('ERPRout_TCGA') % Add the new directory to the PATH
notinthere = (1:4000)';
parfor rr = 1:rtimes % Run on parallel
    rndgntr = randi(1e8); % Set the rng seed for each iteration 
    rng(rndgntr);
    if ~any(rr==notinthere)
        ...
        parsavef(['ERPRout_TCGA/TLas1_r' num2str(rr)],TLas1,BLas1,a0Vals1,YYpg,YYpgdev); % See the directory naming above
